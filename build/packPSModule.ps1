...
try {
    ...
    # These are scripts that don't inherit our EAP
    Register-PSRepository -Name $tempRepoName -SourceLocation $tempRepoPath -InstallationPolicy Trusted -ErrorAction Stop
    Publish-Module -Repository $tempRepoName -Path $Path -SkipAutomaticTags:$SkipAutomaticTags -ErrorAction Stop
